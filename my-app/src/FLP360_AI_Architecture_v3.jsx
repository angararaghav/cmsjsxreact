    ...
    <div>
      ...
      <Card style={{ marginTop:16 }}>
        ...
        <TableView headers={["Capability","Correctness Definition","Primary Test Method"]} rows={[
          ...
          { cells:["Report Explanation","Narrative mentions the correct numbers, direction (up/down), and correct leg names","Automated: parse numbers from narrative, assert they match source data JSON"] },
    ...
    <div>
      ...
      <Card style={{ marginTop:20, marginBottom:16 }}>
        ...
        <TableView headers={["Test Category","# Questions","Examples","Pass Criteria"]} rows={[
          ...
          { cells:["Multi-table downline queries","35","'Which legs dropped this month', 'Show inactive members'","All returned distributor IDs are subset of known subtree"] },
          { cells:["Rank qualification queries","25","'How close am I to Gold', 'Which legs need more CC'","Numeric gap values match semantic layer calculation exactly"] },
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
        <Card accent={C.green} bg={C.greenBg}>
          ...
          {[
            ...
            <div key={i} style={{ display:"flex", gap:10, marginBottom:10, alignItems:"flex-start",
              ...
              <span style={{ background:s.color, color:"#fff", borderRadius:4,
                padding:"2px 7px", fontSize:10, fontWeight:800, flexShrink:0, marginTop:2 }}>{s.label}</span>
              <div>
        ...
        <Card accent={C.purple} bg={C.purpleBg}>
          ...
          {[
            ...
            { failure:"Missing GROUP BY", example:"Query aggregates without grouping, returns single row instead of per-leg breakdown", fix:"Reference SQL comparison — row count mismatch caught immediately by confidence scorer." },
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:16 }}>
        ...
        <Card accent={C.orange}>
          ...
          {[
            ...
            { mechanism:"Entity grounding check", detail:"Distributor names, leg identifiers, and rank names mentioned in the narrative are verified against the actual result data. If the LLM names 'Maria Santos' in an explanation but Maria Santos' ID is not in the result rows, it's flagged.", color:C.purple },
          ].map((item,i) => (
            <div key={i} style={{ marginBottom:14, padding:12, background:item.color+"08",
              border:"1px solid "+item.color+"25", borderRadius:6 }}>
      ...
      <Card>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
          {[
            ...
            { cat:"Downline / Leg queries", color:C.green, questions:[
              "Which legs dropped volume this month?",
              "Show me inactive members in my downline",
              "Which leg has the most CC this quarter?",
              "How many active members do I have in Leg 2?",
            ]},
            { cat:"Rank queries", color:C.purple, questions:[
              "How close am I to Gold rank?",
              "What CC do I need to maintain Silver?",
              "Which legs are closest to qualifying?",
            ...
            <div key={i} style={{ padding:14, background:cat.color+"06",
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:16 }}>
        <Card accent={C.gold} bg={C.goldBg}>
          ...
          {[
            ...
            <div key={i} style={{ marginBottom:10, padding:10, background:item.color+"08",
