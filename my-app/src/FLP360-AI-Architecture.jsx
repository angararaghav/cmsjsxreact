...
LIMIT 3;`,
  ...
  "explanation": "Comparing March vs February by downline leg.",
...
GRANT SELECT ON SCHEMA ai_views TO ai_service_user;`,
  ...
  nlStep7: `{
  "answer": "March bonus fell 12% ($847 to $745). Primary cause: 30% volume drop in Leg 2 (John Doe 284 to 198 CC).",
  "chart": {
    "type": "grouped_bar",
    "labels": ["Feb","Mar"],
    "series": [
      {"name":"Bonus $","data":[847,745]},
      {"name":"Leg 2 CC","data":[284,198]}
    ]
  },
  "citations": ["bonus_facts: Mar+Feb 2024"],
  "followUp": ["Show John Doe activity","Which other legs declined?"]
}`,
...
GRANT SELECT ON SCHEMA ai_views TO ai_service_user;`,
  ...
  flowStep8: `{
  "answer": "March bonus fell 12% ($847 to $745). Cause: Leg 2 volume dropped 30%.",
...
WHERE (r.current_cc / rq.required_cc) < 1.1
  ...
  insightGrowth: `SELECT d.root_id, d.member_id as leg_id,
       SUM(v.cc_units) as leg_cc,
...
Anomaly: {
  type: "VOLUME_DROP", leg: "D004", pct_change: -28,
  current_cc: 198, prev_cc: 284, inactive_members: 3
}

Output: "Your Leg 2 volume dropped 28% this week (284 to 198 CC),
...
Current: { total:745, personal:312CC,
  legs:[{id:D004,cc:198},{id:D007,cc:114}] }
Previous (Feb): { total:847, personal:318CC,
  legs:[{id:D004,cc:284},{id:D007,cc:118}] }
Rank: Gold requires 500CC + 2 legs at 200+ CC each`,
...
function OverviewSection() {
  ...
  const embeds = [
    ...
    { page:"Downline Network View",  widget:"Find weak legs button — AI flags underperforming branches", color:C.accent },
...
const DIAG_NODES = [
  ...
  { id:"vectordb", x:20,  y:394, w:135, h:42, color:C.teal,    bg:C.tealBg,   label:"Vector DB",            sub:"pgvector on RDS" },
  { id:"semantic", x:175, y:394, w:135, h:42, color:C.teal,    bg:C.tealBg,   label:"Semantic Layer",       sub:"Metrics + Rules" },
  { id:"cache",    x:330, y:394, w:135, h:42, color:C.teal,    bg:C.tealBg,   label:"MLM Tree Cache",       sub:"Redis ElastiCache" },
  { id:"redshift", x:485, y:394, w:155, h:42, color:C.accent,  bg:C.accentBg, label:"Amazon Redshift",      sub:"Data Warehouse" },
  { id:"as400",    x:175, y:466, w:135, h:42, color:C.muted,   bg:"#F1F5F9",  label:"AS400 / DB2",          sub:"via Informatica" },
  { id:"aurora",   x:330, y:466, w:135, h:42, color:C.muted,   bg:"#F1F5F9",  label:"Aurora MySQL",         sub:"via Informatica" },
];
...
const DIAG_DETAIL = {
  ...
  cache:   "Pre-materialized flat list of downline subtree IDs per distributor. Refreshed nightly. O(1) Redis lookup replaces expensive recursive CTEs. KEY 'subtree:D001' TTL 86400s.",
  redshift:"Existing data warehouse. AI layer adds: ai_views schema, separate WLM queue for AI queries, Row-Level Security policies, read-only service account with SELECT-only on ai_views.",
  as400:   "Legacy source system feeding existing Informatica pipelines into Redshift. No changes required for AI integration.",
  aurora:  "Transactional database feeding existing Informatica pipelines into Redshift. No changes required for AI integration.",
};
...
const DIAG_ARROWS = [
  ...
  [87,360,87,394],[87,360,242,394],[87,360,397,394],[87,360,562,394],
  [562,360,87,394],
  [562,436,242,466],[562,436,397,466],
];

function ArchDiagramSection() {
  const [hover, setHover] = useState(null);
  const hNode = hover ? DIAG_NODES.find(n=>n.id===hover) : null;
  return (
    <div>
      <SectionHeader title="AI System Architecture Diagram" tag="Architecture"
        subtitle="Interactive diagram. Hover or click any component to see its role and responsibilities." />
      <div style={{ display:"grid", gridTemplateColumns:"1fr 300px", gap:20 }}>
        <div style={{ background:C.surface, border:"1px solid "+C.border,
          borderRadius:12, padding:20, boxShadow:"0 1px 4px rgba(0,0,0,0.06)", overflowX:"auto" }}>
          <svg width="660" height="530" style={{ fontFamily:"inherit", display:"block" }}>
            <defs>
            ...
            {[
              ...
              { y:384, h:54,  label:"DATA LAYER" },
              { y:456, h:54,  label:"SOURCE DATA" },
            ].map((layer,i) => (
              <g key={i}>
            ...
            {DIAG_NODES.map(n => (
              <g key={n.id} style={{ cursor:"pointer" }}
                ...
                <rect x={n.x} y={n.y} width={n.w} height={n.h} rx={6}
                  fill={hover===n.id ? n.color : n.bg}
                  stroke={n.color} strokeWidth={hover===n.id?2:1.5} />
                <text x={n.x+n.w/2} y={n.y+15} fontSize={10.5} fontWeight="700"
                  fill={hover===n.id?"#fff":n.color} textAnchor="middle">{n.label}</text>
                <text x={n.x+n.w/2} y={n.y+29} fontSize={9}
                  fill={hover===n.id?"rgba(255,255,255,0.75)":C.muted}
        ...
        <div>
          {hNode ? (
            <Card accent={hNode.color} bg={hNode.color+"06"} style={{ minHeight:260 }}>
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12 }}>
                <div style={{ width:10, height:10, borderRadius:"50%", background:hNode.color }} />
                <span style={{ color:hNode.color, fontWeight:800, fontSize:11,
                  textTransform:"uppercase", letterSpacing:"0.08em" }}>{hNode.label}</span>
              </div>
              <p style={{ color:C.textMd, fontSize:12.5, lineHeight:1.75,
                margin:0, whiteSpace:"pre-line" }}>{DIAG_DETAIL[hNode.id]}</p>
            </Card>
          ) : (
            <Card style={{ minHeight:260, display:"flex", alignItems:"center",
              justifyContent:"center", flexDirection:"column", gap:12 }}>
              <div style={{ fontSize:28 }}>👆</div>
              <div style={{ color:C.muted, fontSize:13, textAlign:"center", lineHeight:1.6 }}>
                Hover any component<br/>to see details
              </div>
              <div style={{ marginTop:8 }}>
                {[
                  [C.teal,  "Data Layer"],
                  [C.accent,"API / Frontend"],
                  [C.red,   "Security"],
                  [C.orange,"AI Orchestration"],
                  [C.green, "Query Engines"],
                  [C.gold,  "LLM"],
                ].map(([color,label],i) => (
...
function AISystemSection() {
  ...
  const promptCards = [
    ...
    { type:"Rank Progression Advisor", color:C.green, bg:C.greenBg,
      ...
      output:"Numbered action steps with specific targets per leg",
  ...
  return (
    <div>
      ...
      <Callout type="info">
        ...
        "Your March bonus fell 12% ($847 to $745). Primary cause: 30% volume drop in Leg 2 — John Doe CC units fell from 284 to 198."
      </Callout>
...
function SmartInsightsSection() {
  ...
  const detections = [
    { num:"01", title:"Volume Drop Detection", color:C.red, bg:C.redBg,
      role:"Trigger: CC units drop more than 15% week-over-week or 20% month-over-month for any distributor or leg",
      details:["Example alert: Your Leg 2 volume dropped 28% this week. John Doe and 2 others had no activity."],
    ...
    { num:"03", title:"Rank Qualification Risk", color:C.purple, bg:C.purpleBg,
      ...
      details:["Example alert: You need 188 more CC this month to qualify for Gold rank. You are on track if Leg 2 recovers."],
      code:CODE.insightRankRisk, lang:"sql" },
    { num:"04", title:"Growth Opportunity Detection", color:C.green, bg:C.greenBg,
      role:"Trigger: Legs within 20% of a key threshold (rank qualification, bonus tier, activity target)",
      details:["Example alert: Leg 3 (Maria Santos) needs only 42 more CC to qualify for Silver rank this month."],
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginTop:20 }}>
            {[
              ...
              { title:"LLM-Powered Explanation", color:C.purple, icon:"🧠",
                desc:"Raw anomalies converted to plain-English alerts by the LLM. 'Your Leg 2 CC dropped 30% vs last month, driven by 3 inactive members.'" },
        ...
        <div>
          <Card style={{ marginBottom:16 }}>
            ...
            <TableView headers={["Alert Type","Trigger Threshold","Priority","Timing"]} rows={[
              { cells:["Volume Drop (leg)",">30% week-over-week","HIGH","Hourly check"] },
              ...
              { hl:true, cells:["Growth Opportunity","Leg within 15% of qualifying","LOW (positive)","Nightly"] },
            ]} />
          </Card>
          <Card>
            ...
            {[
              { alert:"Volume Drop Alert", color:C.red,
                followUps:["Show me which members in Leg 2 were inactive this week","Compare Leg 2 volume for the last 4 weeks","What was Leg 2 best performing month this year?"] },
              { alert:"Rank Risk Alert", color:C.purple,
                followUps:["How many CC do I need in each leg to maintain Gold?","Show my personal volume trend for the last 6 months","Which legs are closest to hitting their targets?"] },
              { alert:"Growth Opportunity Alert", color:C.green,
                followUps:["What does Maria Santos need to qualify for Silver?","Show Leg 3 activity for the past 30 days","What support has worked for other legs that qualified?"] },
            ].map((a,i) => (
        ...
        <div>
          ...
          <Card style={{ marginBottom:16 }}>
            ...
            {[
              ...
              { input:"What about the other legs?", resolution:"'other legs' — last query was about Leg 2 — implies all direct downline except D004", output:"Queries all direct downline EXCEPT D004" },
            ].map((r,i) => (
          ...
          {[
            { title:"Performance Investigation Flow", color:C.accent,
              turns:[
                ...
                { role:"ai",   msg:"Your March bonus fell 12% ($847 to $745). Primary cause: 30% volume drop in Leg 2 — John Doe CC units fell from 284 to 198." },
                ...
                { role:"user", msg:"What about the other legs?" },
                { role:"ai",   msg:"Your other legs performed well: Leg 1 (Maria Santos) was up 8%, Leg 3 (Carlos Vega) stable at 142 CC." },
              ]},
            { title:"Rank Planning Flow", color:C.green,
              turns:[
                { role:"user", msg:"How close am I to Gold rank?" },
                { role:"ai",   msg:"You need 188 more CC for Gold (500 required, you have 312). Leg 2 is only 2 CC away from the 200-leg requirement!" },
                { role:"user", msg:"Which leg should I focus on?" },
                { role:"ai",   msg:"Focus Leg 2 first — they only need 2 CC to hit 200. After that, your personal volume gap of 186 CC is the remaining blocker." },
              ]},
          ].map((flow,fi) => (
        ...
        <div>
          ...
          <Card style={{ marginTop:16 }}>
            ...
            <TableView headers={["Report","Data Passed to AI","Comparison Period","Special Instructions"]} rows={[
              { cells:["Bonus Summary","Total bonus, by type, by leg","Prior month + same month last year","Highlight any leg that dropped more than 15%"] },
              { cells:["Downline Network","Active/inactive counts, CC per leg, depth","Prior month activity","Flag inactive members by name if fewer than 5"] },
              ...
              { cells:["New Member Report","New members, sponsor, first order date","Same period last month","Highlight fastest-growing legs"] },
            ]} />
          </Card>
        </div>
      )}
        ...
        <div>
          {[
            { report:"Bonus Summary — March 2024", color:C.accent,
              explanation:"Your March bonus came in at $745, a 12% decrease from February's $847. The primary driver was a significant volume reduction in your second downline leg — that leg contributed 198 Case Credits this month versus 284 in February, a drop of 30%.\n\nYour personal volume remained stable at 312 CC, and your first leg performed well, up 8% month-over-month. The issue is concentrated in Leg 2.\n\nRecommendation: Reach out to your Leg 2 distributor about their March activity. Also, you are currently 188 CC short of Gold rank — if Leg 2 returns to February levels, you would be within 90 CC of qualifying.",
              followUps:["Show me Leg 2 activity for the past 3 months","What do I need to qualify for Gold this month?"] },
  ...
  return (
    <div>
      <SectionHeader title="Data Architecture for AI" tag="Data"
        subtitle="How to build the Vector DB, Semantic Layer, MLM tree cache, and Row-Level Security alongside your existing Redshift warehouse." />
      <Tabs tabs={[
        { id:"overview", label:"Data Flow" },
        { id:"vectordb",  label:"Vector DB Setup" },
        { id:"semantic",  label:"Semantic Layer" },
        { id:"mlm",       label:"MLM Tree Strategy" },
        { id:"rls",       label:"Row-Level Security" },
      ]} active={tab} onChange={setTab} />
      {tab==="overview" && (
        <div>
          <Card style={{ marginBottom:16 }}>
            <CardTitle color={C.accent}>Existing Pipeline (unchanged)</CardTitle>
            {[["AS400 / IBM DB2","Amazon Redshift DW"],["Aurora MySQL","Amazon Redshift DW"]].map((row,i) => (
              <div key={i} style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap", marginBottom:8 }}>
                <Pill color={C.muted}>{row[0]}</Pill>
                <span style={{ color:C.muted }}>→ Informatica →</span>
                <Pill color={C.accent}>{row[1]}</Pill>
              </div>
            ))}
          </Card>
          <Card>
            <CardTitle color={C.green}>3 New AI Data Components</CardTitle>
            {[
              ...
              { title:"3. MLM Tree Cache (Redis ElastiCache)", color:C.gold,
                where:"ElastiCache Redis in same VPC",
                stores:"Pre-computed flat list of all subtree IDs per distributor (refreshed nightly)",
                when:"Every AI query: O(1) scope lookup for the WHERE distributor_id IN predicate" },
            ].map((item,i) => (
      ...
      {tab==="mlm" && (
        <div>
          <Callout type="warn">
            Recursive CTE to traverse a 10-level, 10K-node MLM tree at every query is very slow. Solution: nightly batch pre-materialization into Redis.
          </Callout>
          <Card style={{ marginTop:20, marginBottom:16 }}>
            <CardTitle color={C.gold}>Nightly Pre-Materialization Job</CardTitle>
            <CodeBlock code={CODE.mlmBatch} lang="sql" />
          </Card>
          <Card>
            <CardTitle color={C.accent}>O(1) Runtime Lookup</CardTitle>
            <CodeBlock code={CODE.mlmLookup} lang="java" />
          </Card>
        </div>
      )}
      {tab==="rls" && (
...
function CostSection() {
  ...
  const infra = {
    "10k":  { redshift:130,  vector:80,  redis:50,  api:30,  eng:6000 },
    "50k":  { redshift:450,  vector:200, redis:120, api:120, eng:6500 },
    "100k": { redshift:900,  vector:400, redis:280, api:220, eng:10000 },
  };
  ...
  return (
    <div>
      <SectionHeader title="Cost Analysis" tag="Cost"
        subtitle="Monthly cost estimates across LLM providers. Toggle scale to recalculate. Assumes 1,500 input and 500 output tokens per query." />
      ...
      <Card style={{ marginBottom:18 }}>
        ...
        <TableView headers={["Component","AWS Service","Cost/mo","Notes"]}
          rows={[
            ...
            { cells:["MLM Tree Cache","ElastiCache Redis","$"+inf.redis+"/mo","r7g.large"] },
